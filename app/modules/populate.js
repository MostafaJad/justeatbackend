...
module.exports.users = async () => {
  ...
  const serano = {
    email: "p2@gmail.com",
    password: "123",
    companyName: "Serano Bakery",
    address: "830 Pape Ave",
    city: "Toronto",
    postalCode: "M4K 3T5",
    longitude: 43.682893,
    latitude: -79.346803,
    phoneNumber: "6476161113",
    role: "Partner"
  };
  ...
  const greenhouse = {
    email: "p4@gmail.com",
    password: "123",
    companyName: "Greenhouse Juice Co",
    address: "5 Macpherson Ave",
    city: "Toronto",
    postalCode: "M5R 1W7",
    longitude: 43.678943,
    latitude: -79.390782,
    phoneNumber: "6476161111",
    role: "Partner"
  };

  return UserDao.createAll([admin, customer, partner, serano, pizzaHut, greenhouse]);
};
...
module.exports.updatePartner = async () => {
  const serano = await PartnerDao.findOne({
    companyName: "Serano Bakery"
  });
  const pizza = await PartnerDao.findOne({
    companyName: "Pizza Hut"
  });
  const greenhouse = await PartnerDao.findOne({
    companyName: "Greenhouse Juice Co"
  });

  await PartnerDao.updateOne({
    _id: serano._id
  }, {
    $set: {
      dishes: ((await DishDao.findAll({ partner: serano._id })).map(x => x.id))
    }
  });
  ...
  await PartnerDao.updateOne({
    ...
    $set: {
      dishes: ((await DishDao.findAll({ partner: pizza._id })).map(x => x.id))
    }
  });
  ...
  await PartnerDao.updateOne({
    ...
    $set: {
      dishes: ((await DishDao.findAll({ partner: greenhouse._id })).map(x => x.id))
    }
  });
}
