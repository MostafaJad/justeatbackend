...
describe("Auth", function () {
    ...
    describe("Sign Up", function () {
        ...
        it("should return error message on invalid email field", async () => {
            ...
            const expectedMessage = `user validation failed: email: Please fill a valid email address`;
            const response = await request({
                uri: `${api}/signup`,
                resolveWithFullResponse: true,
                json: true,
        ...
        it("should return error message on invalid phoneNumber field", async () => {
            ...
            const expectedMessage =
                `user validation failed: phoneNumber: ` +
        ...
        it("should return error as the user is already registered", async () => {
            let expectedMessage = "E11000 duplicate key error collection: test-justeatit.users index: phoneNumber_1 dup key: { phoneNumber: 4163211234.0 }";
            const response = await request({
                uri: `${api}/signup`,
                resolveWithFullResponse: true,
                json: true,
